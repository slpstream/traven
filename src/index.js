...
export class TravenEditorElement extends HTMLElement {
  ...
  static get observedAttributes() {
    return ["name", "theme", "read-only", "vim-mode", "toolbar-mode", "line-numbers", "toolbar"];
  }
  ...
  connectedCallback() {
    ...
    if (this.hasAttribute("read-only")) options.readOnly = this.hasAttribute("read-only") && this.getAttribute("read-only") !== "false";
    if (this.hasAttribute("vim-mode")) options.vimMode = this.hasAttribute("vim-mode") && this.getAttribute("vim-mode") !== "false";
    ...
    if (this.hasAttribute("toolbar")) {
      ...
      if (tbAttr === "false") {
        ...
        options.toolbar = tbAttr.split(",").map(s => s.trim()).filter(Boolean);
      }
    }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    switch (name) {
      ...
      case "vim-mode":
        ...
        break;
      case "toolbar":
        console.warn("Traven: 'toolbar' is a construction-only attribute. Ignoring dynamic change.");
        break;
    }
  }

  get value() {
    return this._editor ? this._editor.getValue() : this._hiddenTextarea ? this._hiddenTextarea.value : "";
  }
...
if (typeof window !== "undefined" && window.customElements && !window.customElements.get("traven-editor")) {
